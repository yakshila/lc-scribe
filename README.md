...
| 关键节点提醒 | AC 提醒 / N 分钟未 AC 卡壳提醒(可配阈值)/ 每日到期复习提醒 |
| AI 笔记生成 | AC 后一键(或自动)生成结构化笔记:思路 / 复杂度 / 经验提炼 / AI 补充 |
...
solving       开始/AC 时刻 / 用时 / 提交次数 / 一次 AC / 语言   ← 插件采集
approach      直觉 / 解法 / 算法 / 数据结构 / 复杂度           ← 用户可编辑,Agent 辅助
code          语言 / AC 代码 / 关键行标注                      ← Agent 标注
insights      踩坑 / 收获 / 可复用模式 / 相关题                ← Agent 辅助提炼
...
项目无构建步骤,纯原生 ES module + IIFE,改完代码在 `chrome://extensions` 点「重新加载」即可生效。
